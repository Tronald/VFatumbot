...
namespace Imgur.API.Models.Impl
{
  ...
  public class GalleryProfile : IGalleryProfile
  {
    ...
    /// <summary>Total number of images submitted by the user.</summary>
    [JsonProperty("total_gallery_submissions")]
    public virtual int TotalGallerySubmissions { get; set; }

    /// <summary>A list of trophies that the user has.</summary>
    [JsonConverter(typeof (TypeConverter<IEnumerable<Trophy>>))]
    public virtual IEnumerable<ITrophy> Trophies { get; set; } = (IEnumerable<ITrophy>) new List<ITrophy>();
  }
}
